 ...
 *
 * clearRecord():最高記録消す。
 * setThreshold(val):差分検出閾値設定
 * disableDiffRenderTest():yおした時のやつ。差分検出の確認用レンダリングを中止。
 *
 * > pでwebcamだして投影位置にガイドポインタをあわせてuでキャプチャする。
 * > ただしく出来てること確認してpで戻る。差分検出はハウリングするので、yでoffにする。
 * > iでターゲット配置してsでスタート。終わるか中断はr。
 * > setThreshold関数で差分検出の閾値を調整できる。defaultは0.02。
 *
 *
 *
 * 仕様：
